...
export default function Projects() {
  ...
  return (
    <section id="projects" ref={sectionRef} className="py-24 md:py-36 bg-[#0A0A0A]">
      ...
      {/* Horizontal scrollable track — native scroll on all devices */}
      <div
        className="flex gap-5 px-6 md:px-12 overflow-x-auto pb-6"
        style={{
          scrollbarWidth: "none",
      ...
      >
        {/* Hide webkit scrollbar via inline style equivalent */}
        <style>{`.projects-track::-webkit-scrollbar { display: none; }`}</style>

        {projects.map((project, i) => (
          <motion.div
            key={project.name}
            className="relative flex-shrink-0 w-[80vw] sm:w-[340px] md:w-[380px] h-[460px] md:h-[520px] rounded-2xl overflow-hidden group"
          ...
          >
            ...
            {/* Default bottom info — fades out on hover/active */}
            <div className="absolute bottom-0 left-0 right-0 p-8 z-10 transition-opacity duration-300 group-hover:opacity-0 group-active:opacity-0 pointer-events-none">
            ...
            <div className="absolute inset-0 z-20 bg-[#F7F7F5] flex flex-col justify-end p-8 translate-y-full group-hover:translate-y-0 group-active:translate-y-0 transition-transform duration-500 ease-[cubic-bezier(0.25,0.46,0.45,0.94)]">
